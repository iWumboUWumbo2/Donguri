...
//  Donguri
//
//  Ported from Hoshi Reader (BookStorage -> AppFileStorage).
...
class DictionaryManager {
    ...
    func getDictionariesFromStorage(type: DictionaryType) throws -> [DictionaryInfo] {
        ...
        .compactMap {
            ...
            guard values.isDirectory == true else {
                try? AppFileStorage.delete(at: $0)
                return nil
            }
            guard let index = AppFileStorage.load(DictionaryIndex.self, from: $0.appendingPathComponent("index.json")) else {
                try? AppFileStorage.delete(at: $0)
                return nil
            }
        ...
        Task.detached {
            ...
            do {
                for (name, url, type) in recommendedDictionaries {
                    ...
                    let importResult = DictionaryImporter.import(
                        zipPath: temp.path(percentEncoded: false),
                        outputDir: destinationPath
                    )

                    if !importResult.success {
                        throw ImportError.failed(dictionary: remoteIndex.title, reason: importResult.error)
                    }
                }
    ...
    func importDictionary(from urls: [URL]) {
        ...
        Task.detached {
            ...
            for url in urls {
                ...
                let importResult = DictionaryImporter.import(
                    zipPath: url.path(percentEncoded: false),
                    outputDir: FileManager.default.temporaryDirectory.path(percentEncoded: false)
                )

                if importResult.success {
                    let title = importResult.title
                    let temp = FileManager.default.temporaryDirectory
                        .appendingPathComponent(String(title))
                    defer { try? FileManager.default.removeItem(at: temp) }
                    if importResult.termCount > 0 {
                        try await AppFileStorage.copyFile(from: temp, to: "Dictionaries/\(DictionaryType.term.rawValue)/\(title)")
                    }
                    if importResult.freqCount > 0 {
                        try await AppFileStorage.copyFile(from: temp, to: "Dictionaries/\(DictionaryType.frequency.rawValue)/\(title)")
                    }
                    if importResult.pitchCount > 0 {
                        try await AppFileStorage.copyFile(from: temp, to: "Dictionaries/\(DictionaryType.pitch.rawValue)/\(title)")
                    }
        ...
        Task.detached {
            ...
            for (dictionary, type) in dictionaries {
                ...
                do {
                    ...
                    let importResult = DictionaryImporter.import(
                        zipPath: temp.path(percentEncoded: false),
                        outputDir: tempDir.path(percentEncoded: false)
                    )
    ...
    func deleteDictionary(indexSet: IndexSet, type: DictionaryType) {
        ...
        case .term:
            for index in indexSet {
                ...
                try? AppFileStorage.delete(at: dictionary.path)
    ...
    private static func getDictionariesDirectory() throws -> URL {
        try AppFileStorage.getAppDirectory().appendingPathComponent("Dictionaries")
    }

    nonisolated enum ImportError: LocalizedError {
        case failed(dictionary: String, reason: String)

        var errorDescription: String? {
            switch self {
            case .failed(let dictionary, let reason):
                reason.isEmpty ? "\(dictionary): import failed" : "\(dictionary): \(reason)"
            }
        }
    }
